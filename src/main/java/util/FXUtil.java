...
public class FXUtil {
    ...
    public static<T> void autoCompleteComboBoxPlus(JFXComboBox<T> comboBox, AutoCompleteComparator<T> comparatorMethod) {
        ...
        comboBox.getEditor().focusedProperty().addListener((observable,oldValue,newValue) -> {
            ...
            if(oldValue)comboBox.setItems(data);
        });
    ...
    public static<T> T getComboBoxValue(JFXComboBox<T> comboBox){
        if (comboBox.getSelectionModel().getSelectedIndex() < 0) {
            return null;
        } else {
